#!/usr/bin/env python3
"""
BiofilmPoroelasticSim.py — publication-ready version (v3.0.1)
=============================================================

Companion code for:
"Interfacial Drainage Regime as a Coordinate Linking Biofilm
 Mechanics and Transport"

Scope
-----
Generates Figure 2 and Figure 3 as single numbered artwork files, plus
a supplementary validation figure. Figures 1 and 4 are conceptual or
schematic and are not generated by this script.

Artwork policy (v3.0.1)
-----------------------
Each numbered figure is delivered as ONE file, not one file per panel,
in line with standard journal artwork submission ("submit each image
as a separate file"; multi-panel figures remain single numbered
illustrations). Internally, the original panel blocks are rendered at
their established dimensions and then assembled vertically into a
single composite:

    Figure_2  = [A B] over [C D]
    Figure_3  = [A] over [B C]

Every numbered figure is written as BOTH PNG and LZW-compressed TIFF.

    Figures 2 and 3 combine line graphics with colour maps and are
    exported at 500 dpi (combination artwork minimum).
    The line-only supplementary validation figure is exported at
    1000 dpi (bitmapped line-drawing minimum).
...
where p is excess pore pressure, D_p is poroelastic diffusivity,
k is intrinsic permeability, M_c is the drained constrained modulus,
and eta is pore-fluid viscosity.
...
-----------------------
Hard sink: beta -> infinity, implemented as Dirichlet p = 0.
Leaky sink: finite beta, implemented as a mesh-consistent Robin-type
pressure-relief term. The sink connectivity number is

    Bi_sink = beta s / D_p.

Validation
----------
The numerical solution is checked against the exact Terzaghi series for
open-top/sealed-base consolidation and by grid refinement for both hard
and leaky sinks.

Dependencies
------------
Python >= 3.8, NumPy, SciPy, Matplotlib, Pillow.
...
# Elsevier full-page width for combination artwork at 500 dpi.
FULL_PAGE_WIDTH_PX = 3740
...
def save_figure_formats(fig: plt.Figure,
    ...
    fig.savefig(png_path, format="png", **common)
    fig.savefig(
        tiff_path,
        format="tiff",
...
def compose_vertical(block_paths: Sequence[str],
    ...
    Each block is resized proportionally to the same final width. A final
    width of 3740 pixels corresponds to full-page combination artwork at
    500 dpi. This assembly step preserves the internal proportions of each
    original panel block; it does not alter panel layout or aspect ratio.
    """
    if not block_paths:
    ...
    for path in block_paths:
        ...
        new_height = int(round(
            image.height * target_width_px / image.width
        ))

        image = image.resize(
            (target_width_px, new_height),
            resample=resampling
        )
        images.append(image)
    ...
    canvas.save(png_path, format="PNG", dpi=(dpi, dpi), optimize=True)
    canvas.save(
        tiff_path, format="TIFF", dpi=(dpi, dpi), compression="tiff_lzw"
    )

    print(
        f"  [Composite PNG saved]  {png_path} "
        f"({canvas.width} x {canvas.height} px)"
    )
    print(
        f"  [Composite TIFF saved] {tiff_path} "
        f"({canvas.width} x {canvas.height} px)"
    )
...
def figure_2_channel_demo(results: Dict,
    ...
    """Generate Figure 2A-B: channel spacing and pressure relaxation."""
    # INCREASED figsize Y-dimension from 5.0 to 6.0
    ...
    ax.set_ylabel(r"Remaining normalised excess pore pressure",
                  fontsize=12, fontweight="bold")
    ax.set_title(
        "A. Pore-pressure relaxation vs. channel spacing",
        fontsize=17, fontweight="normal"  # +2 -> 16
    )
...
def figure_2_heatmap(L: float = 200e-6,
    ...
    # INCREASED figsize Y-dimension from 5.5 to 6.2
...
LITERATURE_DATA = [
    {"label": "Klapper 02", "L_um": 100, "t_s": 1.0,
     "marker": "D", "color": "#FDAE61", "cat": "theoretical",
     "role": "viscoelastic-fluid model; symbolic protocol scale"},
...
def figure_3A(save_path: Optional[str] = None,
              csv_path: Optional[str] = None) -> plt.Figure:
    """Generate Figure 3A: drainage-regime map."""
    L_arr = np.logspace(0, 3, 55) * 1e-6
    t_arr = np.logspace(-2, 3.3, 55)

    k_low, k_high = 1e-16, 1e-14
    M_c, eta = 1e4, 1e-3
    ...
    for pt in LITERATURE_DATA:
        ...
        label_offset = pt.get("offset", (6, 6))
    ...
    if csv_path:
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["label", "L_eff_um", "t_obs_s", "category", "role"])
            for pt in LITERATURE_DATA:
                writer.writerow([
...
def figure_3BC(save_path: Optional[str] = None,
    ...
    """
    Generate Figure 3B-C.
        B. Geometry scaling: tau ~ L_eff^2.
        C. Permeability scaling: tau_drain ~ 1/k.
    """
    ...
    k_low_case, k_high_case = 1e-16, 1e-14
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 5.8))   # <-- SLIGHTLY TALLER
    ...
    ax.set_xlabel(r"$L_{\mathrm{eff}}$ ($\mathrm{\mu}$m)",
                  fontsize=14, fontweight="bold")
    ax.set_ylabel(r"$\tau_{1/e}$ (s)", fontsize=14, fontweight="bold")
    ax.set_title(r"B. Geometry scaling: $\tau\propto L_{\mathrm{eff}}^2$",
                 fontsize=17, fontweight="normal")  # +2 -> 16
    ...
    ax.annotate("matrix\npermeability shift",
                ...
                fontsize=9)
    ax.set_xlabel(r"Biofilm matrix permeability $k$ (m$^2$)",
                  fontsize=14, fontweight="bold")
    ax.set_ylabel(r"$\tau_{\mathrm{drain}}$ (s)",
                  fontsize=14, fontweight="bold")
    ax.set_title(r"C. Conditional permeability scaling: $\tau_{\mathrm{drain}}\propto 1/k$",
                 fontsize=17, fontweight="normal")
    ax.legend(fontsize=9.5)
    ax.grid(alpha=0.3, which="both")
...
def figure_validation(save_path: Optional[str] = None) -> plt.Figure:
    ...
    ax.set_title("A. Numerical vs. exact Terzaghi",
                 fontsize=16, fontweight="normal")  # +2 -> 16
...
def main():
    ...
    print("Generating Figure 2, Figure 3, and Supplementary Figure S1")
    print("Each numbered figure is saved as a single PNG + TIFF file")
    print("Figures 1 and 4 are conceptual or schematic and are not generated")
    print("#" * 80 + "\n")
    ...
    # ------------------------------------------------------------------
    # Figure 2 (assembled from A-B block over C-D block)
    # ------------------------------------------------------------------
    ...
    compose_vertical(
        ...
        gap_px=150,  # INCREASED SPACING BETWEEN A-B AND C-D PANELS
        dpi=ARTWORK_DPI,
        remove_blocks=True
    )

    # ------------------------------------------------------------------
    # Figure 3 (assembled from A block over B-C block)
    # ------------------------------------------------------------------
    print("\n[Figure 3] Generating complete three-panel artwork...")

    fig3_a = figure_3A(
        save_path=None,
        csv_path=out("Figure_3A_literature_points.csv")
    )
    ...
    fig3_bc = figure_3BC(
        save_path=None,
        csv_path_B=out("Figure_3B_tau_vs_L.csv"),
        csv_path_C=out("Figure_3C_tau_vs_k.csv")
    )
    block3_bc = save_working_block(fig3_bc, "_Figure_3_BC_working.png")
    plt.close(fig3_bc)
    ...
    compose_vertical(
        ...
        stem="Figure_3",
        target_width_px=FULL_PAGE_WIDTH_PX,
        gap_px=150,             # <-- MATCHES FIGURE 2 SPACING
        dpi=ARTWORK_DPI,
        remove_blocks=True
    )
    ...
    print("Figures 1 and 4 must be supplied separately from their source tools.")
    print("#" * 80 + "\n")
